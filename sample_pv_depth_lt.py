...
if __name__ == '__main__':
    ...
    # Extract pixel-to-normalized coordinate mapping from calibration data ----
    uv2xy = calibration_lt.uv2xy
    # Compute ray directions for each pixel and depth scaling factor ----------
    xy1, scale = hl2ss_3dcv.rm_depth_compute_rays(uv2xy, calibration_lt.scale)

    # Prepare ray vectors for zero order hold interpolation -------------------
    # xy1_o: original pixel positions (excluding last row and column)
    xy1_o = xy1[:-1, :-1, :]
    # xy1_d: diagonal offset pixel positions for interpolation coverage
    xy1_d = xy1[1:, 1:, :]
    ...
    while (not listener.pressed()):
        ...
        # Preprocess frames ---------------------------------------------------
        # Extract raw depth data from RM Depth Long Throw sensor
        depth = data_lt.payload.depth
        # Normalize depth values from sensor units to meters using calibration scale
        z     = hl2ss_3dcv.rm_depth_normalize(depth, scale)
        # Extract RGB color image from PV (Personal Video) camera
